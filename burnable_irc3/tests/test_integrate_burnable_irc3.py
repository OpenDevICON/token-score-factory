...
class TestTest(IconIntegrateTestBase):
    ...
    def call_burn(self,params, wallet):
        _call_transaction = CallTransactionBuilder() \
            .from_(wallet.get_address()) \
            .to(self._score_address) \
            .step_limit(100_000_000_000) \
    ...
    def test_call_mint_and_burn(self):
        ...
        tx_result_new = self.call_mint(newmint_params, self._test1)
        ...
        tx_burn = self.call_burn(burn_param, self._test1)
        afterburn = self.call_balanceOf(self._test1.get_address())
        self.assertEqual(True, tx_burn['status'])
        self.assertEqual(int(afterburn, 0), int(aftermint, 0) - 1)

        tx_result_afterburn = self.call_mint(newmint_params, self._test1)
        self.assertEqual(True, tx_result_afterburn['status'])


    def test_call_mint_burn_ntimes(self):
        # mint
        for i in range(10):
            mint_params = {
            ...
            tx_result = self.call_mint(mint_params, self._test2)
            self.assertEqual(True, tx_result['status'])
        ...
        for i in range(10):
            ...
            tx_result = self.call_burn(burn_params, self._test2)
            self.assertEqual(True, tx_result['status'])

        afterburn = self.call_balanceOf(self._test2.get_address())
        self.assertEqual(0, int(afterburn, 0))
    ...
    def test_call_burn(self):
        burn_params = {
            '_tokenId': 1
        }

        self.mint()        
        b4burnn = self.call_balanceOf(self._test1.get_address())
        tx_burnn = self.call_burn(burn_params, self._test1)
        self.assertEqual(True, tx_burnn['status'])
        afterburnn = self.call_balanceOf(self._test1.get_address())
        self.assertEqual(int(afterburnn, 0), int(b4burnn, 0) - 1)
        ...
        # approved owners can burn the token on behalf of owners as well
        tx_burn = self.call_burn(burn_params, self._test2)
        self.assertEqual(True, tx_burn['status'])
