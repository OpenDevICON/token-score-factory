...
class TestTest(IconIntegrateTestBase):
    ...
    def run_tokenRecovery(self, params, wallet):
        _call_transaction = CallTransactionBuilder() \
            .from_(wallet.get_address()) \
            .to(self._score_address) \
            .step_limit(100_000_000_000) \
    ...
    def test_pause(self):
        ...
        tx_result_mint = self.run_mint(param2, self._test1)
        self.assertEqual(False, tx_result_mint['status'])

        tx_result_burn = self.run_burn(param2, self._test1)
    ...
    def conditions_for_recovery(self):
        transfer_params = {
        ...
        test1_bal_b4 = self.run_balanceOf(self._test1.get_address())
        score_bal_b4 = self.run_balanceOf(self._score_address)

        tx_result_transfer = self.run_transfer(transfer_params)

        test1_bal_after = self.run_balanceOf(self._test1.get_address())
        score_bal_after = self.run_balanceOf(self._score_address)
        
        self.assertEqual(transfer_params['_value'], int(test1_bal_b4, 0) - int(test1_bal_after, 0))
        self.assertEqual(transfer_params['_value'], int(score_bal_after, 0) - int(score_bal_b4, 0))

        self.assertEqual(True, tx_result_transfer['status'])

    def test_tokenRecovery(self):
        # by owner
        self.conditions_for_recovery()
        test1_bal_after = self.run_balanceOf(self._test1.get_address())
        score_bal_after = self.run_balanceOf(self._score_address)

        params = {
            "_from" : self._score_address,
            "_value" : 10 * 10 ** 18
        }        

        tx_res = self.run_tokenRecovery(params, self._test1)
        self.assertEqual(True, tx_res['status'])

        test1_bal_after_recovery = self.run_balanceOf(self._test1.get_address())
        score_bal_after_recovery = self.run_balanceOf(self._score_address)
        self.assertEqual(params['_value'], int(test1_bal_after_recovery, 0) - int(test1_bal_after, 0))
        self.assertEqual(params['_value'], int(score_bal_after, 0) - int(score_bal_after_recovery, 0))

    def test_tokenRecovery_by_others(self):
        # not by owner
        self.conditions_for_recovery()
        ...
        tx_res = self.run_tokenRecovery(params, self._test2)

        self.assertEqual(False, tx_res['status'])
        self.assertEqual('Token can be recovered by owner only', tx_res['failure']['message'])
